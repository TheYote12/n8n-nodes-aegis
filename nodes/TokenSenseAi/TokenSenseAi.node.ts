...
export class TokenSenseAi implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				default: 'gpt-image-1',
				options: [
					{ name: 'DALL-E 2 (Deprecated)', value: 'dall-e-2' },
					{ name: 'DALL-E 3 (Deprecated)', value: 'dall-e-3' },
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const authRequest = async (opts: IHttpRequestOptions) =>
			this.helpers.httpRequestWithAuthentication.call(this, 'tokenSenseApi', opts);
			...
			try {
				...
				if (operation === 'chatCompletion') {
					...
					const response = await authRequest({
						...
						body,
						// IMPORTANT: n8n's helper understands this content-type and
						// serialises the body map above into a multipart stream. Do
						// NOT import the `form-data` npm package — that dependency
						// was removed in Sprint 2.6 and is blocked by the community
						// nodes `no-restricted-imports` rule.
						headers: { 'Content-Type': 'multipart/form-data' },
						returnFullResponse: true,
					});

					// CONTRACT: with returnFullResponse: true, n8n returns the
